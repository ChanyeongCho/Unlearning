...
class Generator(nn.Module):
    def __init__(self, z_dim=100, img_shape=(3, 32, 32), ngf=64):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(ngf * 2, img_shape[0], 4, 2, 1, bias=False),
            nn.Tanh()
            # state size. (nc) x 32 x 32 (64×64 대신 32×32)
        )
...
class Discriminator(nn.Module):
    def __init__(self, img_shape=(3, 32, 32), ndf=64):
        super(Discriminator, self).__init__()
        self.img_shape = img_shape
        
        # 32×32 입력으로 조정
        self.main = nn.Sequential(
            # input is (nc) x 32 x 32 (64×64 대신 32×32)
